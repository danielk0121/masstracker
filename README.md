...
- 삼성 헬스 데이터와 연동
- 운동시간, 소모 칼로리 데이터를 차트, 목록에 같이 보여주눈 UI 구상
  - 아마도 꺽은선 그래프와 막대 그래프를 같이 사용하면 좋을듯
- 만약 가능하다면, 주식 일봉 차트처럼 만들 수 있을지도...
  - 소모 칼로리를 매도, 섭취 칼로리를 매수로 취급하고, 체중을 변동 수치로 지정하면...
  - 하지만 체중은 하루에 한번 기록하는데 반해, 칼로리 소모와 섭취는 하루에 2-3번 하는게 일반적
  - 이마저도, 하루 한번 기록도 잘 안해서, 칼로리 데이터는 너무 빈약 할 수 있음
  - 칼로리가 아니라, 체중과 연관지을수 있는 다른 데이터가 필요함
  - 데이터가 많고, 체중과 관련 있을 수 있는... 심박수 ?
  - 관련성이 있는지 조사가 필요함 !
