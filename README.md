...
### 작업 계획: 목표 체중 표시 기능 구현
- **목표**: 사용자가 목표 체중을 설정하고 차트 상에 가로선으로 표시하여 목표 달성 여부를 시각화함.
- **수정 내용**: 
  - 차트 상단에 목표 체중 입력 UI(텍스트 박스, 버튼 4개) 추가.
  - 입력 버튼, 수정 버튼, 적용 버튼 등의 상태 전환 로직 구현.
  - 목표 체중 설정이 없을 시 '올해 평균값'을 기본값으로 자동 계산하여 설정.
  - Chart.js에 목표 체중 가로선(Goal Line) 표시 기능 연동.
- **예상 결과**: 차트 상단에서 목표 체중을 입력하고 적용하면 차트에 가로선이 나타나고 시각적 추적성이 높아짐.

---

### 작업 계획: 주간/월간 평균 탭 1년 기준 표시
- **목표**: 주간평균, 월간평균 버튼 클릭 시 사용자에게 1년치 트렌드를 한눈에 보여주기 위해 표시 기간을 365일로 조정함.
- **수정 내용**: 
  - `prototype/weight-chart.html`의 `getInitialWindow` 함수 수정.
  - `weekly`, `monthly` 모드 시 `min` 값을 현재 시점 기준 365일 전으로 설정.
- **예상 결과**: 해당 버튼 클릭 시 차트가 자동으로 1년치 범위를 비추도록 초기 줌이 조절됨.

---

### 작업 계획: 요약 카드 최저/최고 날짜에 요일 추가
- **목표**: 요약 카드의 최저 및 최고 체중 포인트 날짜에 요일 정보를 추가하여 정보의 직관성을 높임.
- **수정 내용**: 
...
### 작업 계획: CLAUDE 지침을 GEMINI.md로 통합 (완료)
